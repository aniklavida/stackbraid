...
**Deliberately not added:** `Hangfire.Core` — named in `docs/SPEC.md`'s dependency table as the intended durable implementation behind `IJobScheduler` — is still not referenced by any `.csproj`. Hangfire is LGPL-3.0-only and its client is compiled into user code, which AGENTS.md's dependency policy does not allow (only MIT/Apache/BSD may be compiled into user code); the "run as a separate process" allowance does not cover a library linked into the API binary. A durable `IJobScheduler` is instead implemented natively over the already-shipped Postgres, with no new dependency: `PersistentJobScheduler` stores each job in `shared_jobs` (via `Npgsql`, already referenced by `Database/Postgres`), retries with backoff and dead-letters. The RabbitMQ client is likewise not referenced — `IMessageBus` ships with an in-memory fake only. `ClosedXML` (0.104.2, MIT), `QuestPDF` (2024.12.3, QuestPDF Community), and `AWSSDK.S3` (3.7.415.5, Apache-2.0) are referenced by `backends/dotnet/src/Shared` and audited as of 2026-09-24.
...
Hangfire Core (LGPL-3.0-only) is **not** adopted for durable job scheduling behind `IJobScheduler`: its client is compiled into user code, which the AGENTS.md policy restricts to MIT/Apache/BSD. Durable scheduling ships instead as a native Postgres-backed `PersistentJobScheduler` with no new dependency, so Hangfire is not referenced in any `.csproj`. The RabbitMQ client is not referenced either; the messaging abstraction ships with an in-memory fake.
