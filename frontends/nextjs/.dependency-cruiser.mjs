 ...
 * a frontend with no dependency-injection container: `domain/` depends on
 * nothing, `data/` may depend on `domain/` and the generated client,
 * `application/` orchestrates `domain/` and `data/`, and `presentation/`
 * only ever reaches `application/` and `domain/` — never `data/` directly,
 * so a screen can never bypass a use case and call the API on its own.
 *
...
const crossFeatureRules = FEATURES.map(({ app, name }) => ({
  ...
  to: {
    path: `^src/${app}/features/${name}/(?!index\\.(ts|tsx)$).+`,
  },
}));
...
const dependencyCruiserConfig = {
  forbidden: [
    ...
    {
      name: "no-orphans",
      comment: "A file nothing imports and that imports nothing itself is almost always dead — Next.js route files, config and generated files are excluded below.",
      severity: "warn",
      from: {
        orphan: true,
        pathNot: [
          "\\.d\\.ts$",
          "^src/app/", // Next.js route entry points are discovered by convention, never imported.
        ],
      },
      to: {},
    },
  ...
  options: {
    ...
    exclude: {
      path: "^src/components/ui/", // shadcn/ui primitives — vendored UI, not part of the layering this checks.
    },
  },
};
