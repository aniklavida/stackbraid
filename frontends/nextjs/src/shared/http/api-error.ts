...
/**
 * Turns a generated client call's `{ data } | { error }` result into a
 * plain return-or-throw, so every use case reads like ordinary async code
 * instead of re-checking `.error` at every call site.
 */
