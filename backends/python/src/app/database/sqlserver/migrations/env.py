"""Alembic environment for SQL Server — the only place this provider's
migration wiring lives. Runs asynchronously against the same ``aioodbc``
driver the application uses, via ``AsyncEngine.run_sync`` (Alembic's own
documented recipe for an async dialect), so no second, sync-only driver needs
to be installed.
"""
...
# persistence models, since it lives in database/sqlserver/, not in a feature.
