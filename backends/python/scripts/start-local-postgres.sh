...
# Docker, no Testcontainers. Same technique as the .NET backend's own
# scripts/start-local-postgres.sh: initdb's a fresh cluster into a temp
# directory, starts it on a free port, creates one database, and prints a
# SQLAlchemy async DSN on stdout (and nothing else), so it can be captured
# directly:
#
...
# backends/python/.local-postgres-test-state so stop-local-postgres.sh can
...
pg_data_dir="$(mktemp -d "${TMPDIR:-/tmp}/stackbraid-python-test-postgres.XXXXXX")"
...
echo "postgresql+asyncpg://postgres@127.0.0.1:$port/stackbraid_test"
